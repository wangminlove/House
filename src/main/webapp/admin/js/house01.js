    ...
    $('#dg').datagrid({
        ...
        columns: [[
            ...
            {field: 'Operation', title: '操作', width: 100,align:"center",
                formatter: function (value, row, index) {
                    var e = '<a href="#" id="ispass" onclick="Housepass(' +row.id+ ')">审核</a> ';
                    return e;
                }
            }
        ]]
...
function Housepass(id) {
    $.post("HousePass",{"id":id},function (data) {
         if (data.result>0){
             $.messager.alert("提示框","房屋审核状态已更新为已审核!")
             $("#dg").datagrid("reload")
         }else{
             $.messager.alert("提示框","房屋审核操作失败!")
         }
    },"json")
}